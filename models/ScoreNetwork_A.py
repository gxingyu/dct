...
from utilss.graph_utils import mask_adjs, pow_tensor, node_feature_to_matrix, mask_x
...
class ScoreNetworkA(BaselineNetwork):
    ...
    def forward(self, x, adj, flags):
        ...
        node_num = score.shape[1]
        self.mask = torch.ones([node_num, node_num]) - torch.eye(node_num)
        self.mask.unsqueeze_(0)
        self.mask = self.mask.to(score.device)
        score = score * self.mask
